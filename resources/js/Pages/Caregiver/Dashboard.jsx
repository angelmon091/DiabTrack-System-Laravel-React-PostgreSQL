    ...
    return <AuthenticatedLayout>
        ...
        <div className="grid gap-6 xl:grid-cols-[minmax(19rem,1fr)_minmax(0,2fr)]">
            <aside className="order-2 space-y-5 xl:order-1">
                ...
                {patients.length > 0 && <><section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="mb-4 text-xs font-bold uppercase tracking-wide text-slate-500">Pacientes vinculados</h2><div className="space-y-3">{patients.map((patient) => <div key={patient.id} className={`rounded-2xl border-2 p-3 ${patient.selected ? 'border-cyan-500 bg-cyan-50' : 'border-slate-100'}`}><Link href={patient.dashboardUrl} preserveScroll className="flex items-center gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-cyan-600 text-white"><UserRound size={19} /></span><span className="min-w-0"><strong className="block truncate">{patient.name}</strong><span className="block text-xs text-slate-500">Parentesco: {patient.relationship}</span></span></Link><div className="mt-3 flex items-center justify-between border-t border-slate-100 pt-2"><span className="text-xs">Glucosa: <strong>{patient.latestGlucose ?? '--'} mg/dL</strong></span><button type="button" onClick={() => setUnlinkPatient(patient)} className="text-xs font-semibold text-red-600">Desvincular</button></div></div>)}</div></section><section className="rounded-3xl border-2 border-dashed border-slate-200 bg-white p-6 text-center"><UserRound className="mx-auto text-cyan-600" /><p className="mt-3 text-sm text-slate-500">¿Cuidas a alguien más? Vincula otro paciente para acompañar su salud desde aquí.</p><Link href={urls.link} className="mt-4 inline-flex items-center gap-2 rounded-xl bg-cyan-600 px-4 py-2 text-sm font-semibold text-white"><LinkIcon size={16} />Vincular otro paciente</Link></section></>}
                {patients.length === 0 && <EmptySidebarGuide profile="caregiver" />}
            </aside>
            <section className="order-1 space-y-6 xl:order-2">{selectedPatient ? <>
                <header className="flex flex-wrap items-center justify-between gap-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><div className="flex items-center gap-4"><span className="flex h-14 w-14 shrink-0 overflow-hidden items-center justify-center rounded-full border-2 border-cyan-500 bg-cyan-50 text-cyan-700">{selectedPatient.avatarUrl ? <img src={selectedPatient.avatarUrl} alt={selectedPatient.name} className="h-full w-full object-cover" /> : <UserRound size={27} />}</span><div><h2 className="text-2xl font-bold">{selectedPatient.name}</h2><p className="mt-1 text-sm text-slate-500">{selectedPatient.diabetesType} · {selectedPatient.age ?? '--'} años · {selectedPatient.weight ?? '--'} kg</p></div></div><Link href={selectedPatient.vitalCreateUrl} className="rounded-xl bg-gradient-to-br from-cyan-500 to-cyan-600 px-5 py-3 font-semibold text-white shadow-md shadow-cyan-500/20">Registrar datos</Link></header>
                <div className="grid gap-4 sm:grid-cols-3"><DashboardMetricCard label="Última glucosa" value={metrics.latestGlucose} unit="mg/dL" icon={Gauge} tone={metrics.latestGlucose > 140 ? 'red' : 'cyan'} /><DashboardMetricCard label="Tiempo en rango" value={metrics.timeInRange} unit="%" icon={Activity} tone="emerald" /><DashboardMetricCard label="Última HbA1c" value={metrics.latestHba1c} unit="%" icon={HeartPulse} tone="violet" /></div>
                <ChartCard title="Tendencia semanal (glucosa)" labels={metrics.glucoseLabels} values={metrics.glucoseData} />
                <section><h2 className="mb-3 text-sm font-bold uppercase tracking-wide text-slate-500">Registros recientes</h2><Table headers={['Fecha', 'Nivel', 'Momento', 'Estado']}>{recentLogs.length ? recentLogs.map((log) => <tr key={log.id}><td className="px-5 py-4 text-sm">{log.date}</td><td className="px-5 py-4 font-bold text-cyan-700">{log.glucose} mg/dL</td><td className="px-5 py-4"><MeasurementMomentBadge value={log.moment} /></td><td className="px-5 py-4"><GlucoseStatusBadge statusKey={log.statusKey} label={log.status} /></td></tr>) : <tr><td colSpan="4" className="px-5 py-8 text-center text-sm text-slate-500">No hay registros recientes.</td></tr>}</Table></section>
            </> : <EmptyLinkedPatients profile="caregiver" linkUrl={urls.link} />}</section>
        </div>
