...
export default function Summary({ metrics, charts, histories }) {
    ...
    return <AuthenticatedLayout><Head title="Resumen de salud" />
        <header className="mb-6"><h1 className="text-3xl font-extrabold">Resumen de salud</h1><p className="mt-2 text-slate-500">Analiza tus tendencias y registros recientes.</p></header>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><Metric label="Glucosa promedio" value={metrics.avgGlucose} unit="mg/dL" status={metrics.glucoseStatus} /><Metric label="Tiempo en rango" value={metrics.timeInRange} unit="%" /><Metric label="Última HbA1c" value={metrics.latestHba1c} unit="%" /><Metric label="Peso" value={metrics.weight} unit="kg" /><Metric label="Presión promedio" value={`${metrics.avgSystolic || '--'}/${metrics.avgDiastolic || '--'}`} unit="mmHg" status={metrics.bpStatus} /><Metric label="Pulso promedio" value={metrics.avgHeartRate} unit="bpm" status={metrics.hrStatus} /><Metric label="Carbohidratos" value={metrics.totalCarbs} unit="g" /><Metric label="Actividad" value={metrics.activityHours} unit="h" /></div>
