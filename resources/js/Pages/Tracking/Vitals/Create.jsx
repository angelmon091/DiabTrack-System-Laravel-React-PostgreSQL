    ...
    return <AuthenticatedLayout>
        ...
        <section data-testid="vitals-create"><div className="mb-7"><h1 className="text-3xl font-extrabold text-slate-900">Registro de Signos Vitales</h1><p className="mt-2 text-slate-500">Registra tus mediciones corporales para un mejor control</p></div><TrackingNav items={trackingNavigation} active="vitals" />
            <form onSubmit={submit} noValidate className="grid gap-6 lg:grid-cols-[minmax(0,1.45fr)_minmax(300px,0.75fr)]">
                <div className="space-y-6"><div className="space-y-7 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><RangeField id="glucose_level" name="glucose_level" label="Nivel de Glucosa (Azúcar)" value={form.data.glucose_level} unit="mg/dL" min="40" max="300" onChange={(event) => form.setData('glucose_level', Number(event.target.value))} error={form.errors.glucose_level} /><div><p className="mb-3 text-sm font-bold text-slate-700">Presión Arterial (Sistólica / Diastólica) <span className="font-normal text-slate-400">(Opcional)</span></p><div className="grid gap-4 sm:grid-cols-2"><FormInput id="systolic" name="systolic" type="number" min="40" max="250" label="Sistólica" placeholder="Sistólica" value={form.data.systolic} onChange={(event) => form.setData('systolic', event.target.value)} error={form.errors.systolic} /><FormInput id="diastolic" name="diastolic" type="number" min="30" max="150" label="Diastólica" placeholder="Diastólica" value={form.data.diastolic} onChange={(event) => form.setData('diastolic', event.target.value)} error={form.errors.diastolic} /></div></div><RangeField id="heart_rate" name="heart_rate" label="Frecuencia Cardiaca (Opcional)" value={form.data.heart_rate} unit="bpm" min="40" max="200" onChange={(event) => form.setData('heart_rate', Number(event.target.value))} error={form.errors.heart_rate} /><FormInput id="hba1c" name="hba1c" type="number" step="0.1" min="3" max="15" label="Hemoglobina Glicosilada (HbA1c) (Opcional)" placeholder="% de HbA1c" value={form.data.hba1c} onChange={(event) => form.setData('hba1c', event.target.value)} error={form.errors.hba1c} /></div><div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><FormTextarea id="notes" name="notes" rows={4} maxLength={1000} label="Notas Adicionales (Opcional)" placeholder="Ej: Fui a una fiesta y comí pastel..." value={form.data.notes} onChange={(event) => form.setData('notes', event.target.value)} error={form.errors.notes} /></div></div>
