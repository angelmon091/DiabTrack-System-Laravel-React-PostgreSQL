...
export default function FormTextarea({ id, label, error, help, className = '', textareaClassName = '', required = false, ...textareaProps }) {
    const errorId = error ? `${id}-error` : undefined;
    return <div className={className}>
        {label && <label htmlFor={id} className="mb-1.5 block text-sm font-semibold text-slate-500"><span className="inline-flex items-center gap-1.5">{label}{help && <InfoTooltip text={help} />}</span>{required && <span className="ml-1 text-red-600" aria-hidden="true">*</span>}</label>}
